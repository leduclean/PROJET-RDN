...
def test_softmax():
    ...
    assert np.allclose(
        sm,
        np.array(
            [[0.09003057, 0.24472847, 0.66524096], [0.21194156, 0.57611688, 0.21194156]]
        ),
    )
...
def test_predit_classe():
    ...
    assert np.allclose(
        result1, expected1
    ), f"Failed for {Y1}. Expected {expected1}, got {result1}"
    ...
    expected2 = np.array(
        [[0.0, 1.0], [1.0, 0.0], [1, 0]]
    )  # argmax returns first max in case of tie
    result2 = predit_classe(Y2, K2)
    assert np.allclose(
        result2, expected2
    ), f"Failed for {Y2}. Expected {expected2}, got {result2}"
    ...
    assert np.allclose(
        result3, expected3
    ), f"Failed for {Y3}. Expected {expected3}, got {result3}"
...
def test_taux_precision():
    ...
    assert np.isclose(
        result1, expected1
    ), f"Basic case failed. Expected {expected1}%, got {result1}%"
