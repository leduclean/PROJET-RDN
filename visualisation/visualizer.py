...
class Visualiseur:
    ...
    def tracer_progression_entrainement(
        self, donnees_erreur: list, donnees_precision: list
    ) -> None:
        """
        ...
        axe1.plot(
            iterations_erreur, erreurs, label="Erreur de cross-entropie", color="red"
        )
        ...
        axe2.plot(
            iterations_precision,
            precisions,
            label=f"Précision (final = {precisions[-1]})",
            color="green",
        )
    ...
    def tracer_frontieres_decision(
        self,
        poids: np.array,
        biais: np.array,
        X: np.array,
        T: np.array,
        architecure: list = None,
    ):
        """
        ...
        for k in range(K):
            ...
            plt.plot(
                valeurs_x, valeurs_y, color=couleurs[k], label=f"Frontière classe {k}"
            )
        ...
        plt.legend()
        plt.title(
            f"Frontières de décision - Reseau dense d'architecture: {architecure} "
            if architecure
            else "Frontières de décision - Régression logistique multiclasses"
        )
        plt.show()
    ...
    def tracer_multiple(
        ...
        liste_courbes: list,
        dimensions: list = [],
        a_afficher: str = "error",
        modulation_taux: np.ndarray = None,
        nom_carte="viridis",
    ) -> None:
        """
        ...
        couleurs = plt.cm.get_cmap(
            nom_carte, nb_courbes
        )  # Récupération de la carte de couleurs

        # Pour chaque courbe dans la liste
        for i, courbe in enumerate(liste_courbes):
            # Séparation des itérations et des valeurs
            iterations, valeurs = zip(
                *(courbe[0] if a_afficher == "error" else courbe[1])
            )
            ...
            if modulation_taux is not None and modulation_taux.size > 0:
                axe.plot(
                    ...
                    color=couleurs(i),
                    label=(
                        f"lr: {modulation_taux[i]:.6f}, t_final ={valeurs[-1]}"
                        if a_afficher != "error"
                        else f"Architecture: {dimensions[i]}, e_final = {valeurs[-1]}"
                    ),
                )
            else:
                axe.plot(
                    ...
                    color=couleurs(i),
                    label=(
                        f"Architecture: {dimensions[i]}, t_final = {valeurs[-1]}"
                        if a_afficher != "error"
                        else f"Architecture: {dimensions[i]}, e_final = {valeurs[-1]}"
                    ),
                )
        ...
        if a_afficher == "error":
            ...
            if modulation_taux is not None and modulation_taux.size > 0:
                axe.set_title(
                    "Comparaison des erreurs pour différents taux d'apprentissage"
                )
            else:
            ...
            if modulation_taux is not None and modulation_taux.size > 0:
                axe.set_title(
                    "Comparaison des taux de precision pour différents taux d'apprentissage"
                )
            else:
                axe.set_title(
                    "Comparaison des taux de precision pour différentes architectures"
                )
